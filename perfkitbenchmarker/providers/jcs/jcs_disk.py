...
"""Module containing classes related to JCS disks.
"""
...
JCS = 'JCS'
disk.RegisterDiskTypeMap(JCS, DISK_TYPE)


class JcsDiskSpec(disk.BaseDiskSpec):
  """Object holding the information needed to create an JcsDisk."""
  ...
  def __init__(self, iops=None, **kwargs):
    ...
    """
    super(JcsDiskSpec, self).__init__(**kwargs)
    self.iops = iops

  def ApplyFlags(self, flags):
    """Apply flags to the DiskSpec."""
    super(JcsDiskSpec, self).ApplyFlags(flags)
    self.iops = flags.aws_provisioned_iops or self.iops


class JcsDisk(disk.BaseDisk):
  """Object representing an Jcs Disk."""

  _lock = threading.Lock()
  vm_devices = {}

  def __init__(self, disk_spec, zone, machine_type):
    super(JcsDisk, self).__init__(disk_spec)
  ...
  def _Delete(self):
    ...
    delete_cmd = util.JCS_EC2_PREFIX + [
        ...
        'delete-volume',
        '--volume-id=%s' % self.id]
    logging.info('Deleting JCS volume %s. This may fail if the disk is not '
  ...
  def _Exists(self):
    ...
    if("InvalidVolume.NotFound" in stderr):
	return False
    response = json.loads(stdout)
  ...
  def Attach(self, vm):
    ...
    Args:
      vm: The JcsVirtualMachine instance to which the disk will be attached.
    """
    with self._lock:
      self.attached_vm_id = vm.id
      if self.attached_vm_id not in JcsDisk.vm_devices:
        JcsDisk.vm_devices[self.attached_vm_id] = set(
            string.ascii_lowercase) - set(['a'])
      self.device_letter = min(JcsDisk.vm_devices[self.attached_vm_id])
      JcsDisk.vm_devices[self.attached_vm_id].remove(self.device_letter)

    attach_cmd = util.JCS_EC2_PREFIX + [
        'ec2',
        'attach-volume',
        '--instance-id=%s' % self.attached_vm_id,
        '--volume-id=%s' % self.id,
        '--device=%s' % self.GetDevicePath()]
    logging.info('Attaching JCS volume %s. This may fail if the disk is not '
                 'ready, but will be retried.', self.id)
    util.IssueRetryableCommand(attach_cmd)

  def Detach(self):
    """Detaches the disk from a VM."""
    detach_cmd = util.JCS_EC2_PREFIX + [
        'ec2',
        'detach-volume',
        '--instance-id=%s' % self.attached_vm_id,
        '--volume-id=%s' % self.id]
    ...
    with self._lock:
      assert self.attached_vm_id in JcsDisk.vm_devices
      JcsDisk.vm_devices[self.attached_vm_id].add(self.device_letter)
      self.attached_vm_id = None
      self.device_letter = None
