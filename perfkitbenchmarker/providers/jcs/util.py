...
"""Utilities for working with JioCloud Web Services resources."""
...
JCS_EC2_PATH = 'aws' # --endpoint https://compute.ind-west-1.staging.jiocloudservices.com'
JCS_EC2_PREFIX = ['aws', '--endpoint', 'https://compute.ind-west-1.staging.jiocloudservices.com', '--output', 'json']

#JCS_VPC_PATH = '/usr/local/bin/aws --endpoint https://vpc.ind-west-1.staging.jiocloudservices.com'
#JCS_VPC_PREFIX = [JCS_VPC_PATH, '--output', 'json']

JCS_VPC_PREFIX = ['aws', '--endpoint', 'https://vpc.ind-west-1.staging.jiocloudservices.com', '--output', 'json']
...
def IsRegion(zone_or_region):
  ...
  if not re.match(r'[a-z]{2}-[a-z]+-[0-9][a-z]?$', zone_or_region):
    raise ValueError(
        '%s is not a valid JCS zone or region name' % zone_or_region)
...
def AddTags(resource_id, region, **kwargs):
  """Adds tags to an JCS resource created by PerfKitBenchmarker.

  Args:
    resource_id: An extant JCS resource to operate on.
    region: The JCS region 'resource_id' was created in.
    **kwargs: dict. Key-value pairs to set on the instance.
  """
  if not kwargs:
    return

  tag_cmd = JCS_EC2_PREFIX + [
      'ec2',
      'create-tags',
      '--region=%s' % region,
...
@vm_util.Retry()
def IssueRetryableCommand(cmd, env=None, is_shell=None):
  """Tries running the provided command until it succeeds or times out.

  On Windows, the JCS CLI doesn't correctly set the return code when it
  ...
  """
  stdout, stderr, retcode = vm_util.IssueCommand(cmd, env=env, is_shell=is_shell)
  if retcode:
