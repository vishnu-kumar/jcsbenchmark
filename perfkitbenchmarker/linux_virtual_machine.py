...
class BaseLinuxMixin(virtual_machine.BaseOsMixin):
  ...
  def SetupProxy(self):
    ...
    commands.append("echo 'nameserver 8.8.8.8' | sudo tee -a /etc/resolv.conf")
...
class DebianMixin(BaseLinuxMixin):
  ...
  def AptUpdate(self):
    ...
    try:
      ...
      self.RemoteCommand('sudo http_proxy=%s apt-get update'%FLAGS.http_proxy, timeout=300)
    except errors.VirtualMachine.RemoteCommandError as e:
  ...
  def InstallPackages(self, packages):
    ...
    try:
      install_command = ('sudo http_proxy=%s DEBIAN_FRONTEND=\'noninteractive\' '
                         '/usr/bin/apt-get -y install %s' % (FLAGS.http_proxy, packages))
